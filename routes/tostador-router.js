...
router.get('/green-lots', async (req, res) => {
  try {
    ...
    const url = rest('/green_lots') + `?select=id,lot_code,current_kg,harvest_year,variety:varieties(name)&roastery_farm_id=eq.${negocio.farmId}&status=eq.available&current_kg=gt.0&order=created_at.desc`;
